...
```{r}
ggplot(data = rosca) + 
  geom_bar(mapping = aes(treatment, fill = bg_female), position = "fill") + 
  theme(axis.title.x = element_blank()) + 
  theme(axis.title.y = element_blank()) + 
  labs(
    title = "gender distribution vs. treatment",
...
```{r }
ggplot(data = rosca) + 
  geom_bar(mapping = aes(treatment, fill = bg_married), position = "dodge") + 
  theme(axis.title.x = element_blank()) + 
  theme(axis.title.y = element_blank()) + 
  labs(
    title = "gender distribution vs. treatment"
...
``` {r}
  ggplot(data = rosca, mapping = aes(x= factor(treatment),y = fol2_amtinvest)) + 
  geom_boxplot()
```
...
```{r}
ggplot(data = rosca, mapping = aes(x= factor(treatment),y = fol2_amtinvest)) + 
  geom_boxplot() + facet_grid(~bg_married, labeller = as_labeller(label)) + 
  scale_y_continuous(limits = c(0, 1800))
```

```{r}
ggplot(data = rosca, mapping = aes(x= factor(treatment),y = fol2_amtinvest)) + 
  geom_boxplot() + facet_grid(~bg_female, labeller = as_labeller(label)) + 
  scale_y_continuous(limits = c(0, 1800))
```

Medians are very similar, but the range difference are vastly different.
...
Seems like the younger you are, the more amount of investment you invest.

```{r}
  ggplot (data = rosca, mapping = aes(x = bg_b1_age, y =fol2_amtinvest)) + 
    geom_point() + 
    scale_y_continuous(limits = c(0,1500)) + geom_smooth() + facet_grid(~treatment)
```
