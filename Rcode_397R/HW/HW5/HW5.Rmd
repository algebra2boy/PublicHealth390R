...
who2 %>% 
  ...
  # gives it a level and make it a str
  mutate(age = factor(age)) %>% 
  mutate(age = fct_recode(age, "0-14" = "14", "65-" = "65", 
                          "15-24" = "1524", "25-34" = "2534",
                          "35-44" = "3544", "45-54" = "4554",
                          "55-64" = "5564")) %>% 
  mutate(age = fct_collapse(age, "25-44" = c("25-34", "35-44") ,"45-64" = c("45-54", "55-64"))) %>% 
  group_by(year, sex, age) %>% 
  summarise(case = sum(case)) %>%  
  ggplot(mapping = aes(year, case, color = age)) + 
  geom_line() + 
  facet_wrap(~sex) +
  labs(x = "year",
      y = "total",
      title = "total number of cases of TB between female and male")
```
