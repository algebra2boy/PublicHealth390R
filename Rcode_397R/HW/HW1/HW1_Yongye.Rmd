...
```
```{r echo=FALSE}
cat(paste("answer up to 101 is", answer2))
cat(paste("answer up to 1001 is", answer3))
cat(paste("answer up to 100001 is", answer4))
cat("I noticed that as number gets larger and larger, it will get closer to closer the value of pi, which is 3.1415926")
```
...
```{r}
df_size_larger_size = df[df$Size=='large',]
df_size_larger_size
```
...
```{r}
ggplot(data = cars) +
  geom_point(mapping = aes(x = speed, y = dist), color = 'blue')
```
<br>
...
```{r mpg}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = cyl, y = displ), color = "red")
```
<br>
The variable "displ" is the engine displacement in litres and the variable "cyl" is the number of cylinders of the car. Engine displacement by definition is the measure of the cylinder volume. The relationship I noticed here is as cyl increases, displ increases as well in a constant relationship. In other words, the more cylinder volumes it requires, the higher an engine's displacement, and giving the car more power, which means it requires more cylinders to generate those 
mechanical power. The graph also tells us that displ between 
2.5 and 4.2 would need 6 cylinders, and displ between 4 and 7 would need 8 cylinders. 
...
```
<br>
"cty" is city miles per gallon, and "hwy" is the highway miles per gallon. There is a linear relationship between cty and hwy. As cty increases linearly, hwy also increases linearly. I noticed that
driving on the highway takes more miles pers gallon than in city, meaning it costs more gas if driving in city. The reason might be 
there more traffic and congestion area in the city, which involves
many actions like waiting for the signal light and watching out
pedestrian.
...
```{r}
city_miles_per_gallon_most_efficient = max(mpg$cty)
city_model = mpg[mpg$cty==city_miles_per_gallon_most_efficient,]

highway_miles_per_gallon_most_efficient = max(mpg$hwy)
highway_model = mpg[mpg$hwy==highway_miles_per_gallon_most_efficient,]
city_model
highway_model
```
<br>
```{r car_model, echo = FALSE}
print(paste("The most efficient in terms of the city miles per gallon is", city_model$model))
```
```{r highway_model, echo = FALSE}
print(paste("The most efficient in terms of the highway miles per gallon are", highway_model$model[1],
            "and",  highway_model$model[2]))
```
